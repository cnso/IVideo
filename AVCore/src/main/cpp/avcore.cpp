...
int sendVideo(int8_t *buf, int len, long tms) {
    ...
    if (buf[4] == 0x67) {//sps pps
        ...
        if (buf[4] == 0x65) {//关键帧 I 帧
            RTMPPacket *packet = createVideoSPSPPSPackage(live);//发送sps pps
...
Java_com_ivideo_avcore_rtmp_RtmpPush_connect(JNIEnv *env, jobject thiz, jstring url_) {
...
extern "C"
JNIEXPORT jboolean JNICALL
Java_com_ivideo_avcore_rtmp_RtmpPush_sendData(JNIEnv *env, jobject thiz, jbyteArray data_,
